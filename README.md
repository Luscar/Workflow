...
2. **Query externe** : Délègue l'évaluation à `IBpmMediateur.EvaluateDecisionAsync`
...
Chaque nœud peut porter des paramètres statiques (`Dictionary<string, object>`) définis à la conception. Ces paramètres sont transmis automatiquement à `IBpmMediateur` lors de l'exécution.
...
Les paramètres sont passés en dernier argument de `IBpmMediateur` :
...
// 1. Enregistrer l'implémentation IBpmMediateur (requis)
...
### IBpmMediateur (approche directe)
...
> **Note** : Les deux approches sont mutuellement exclusives. `AddCommandHandlers()` enregistre automatiquement un `BpmMediateur` comme `IBpmMediateur`, qui dispatche vers les handlers individuels. Si vous utilisez l'injection de handlers, vous n'avez pas besoin d'implémenter `IBpmMediateur` directement.
...
3. Un `BpmMediateur` est automatiquement enregistré comme `IBpmMediateur`
...
Le `BpmMediateur` maintient un dictionnaire interne indexé par `CommandName` / `QueryName` pour un dispatch en O(1) :
...
- Les nœuds métier appellent des commandes via `IBpmMediateur`
- Les décisions sont évaluées via `IBpmMediateur.EvaluateDecisionAsync`
...
- **Injection de handlers** : les commandes métier et décisions peuvent être implémentées comme des handlers individuels (`ICommandHandler` / `IQueryHandler`), découverts automatiquement via `AddCommandHandlers()` et dispatchés par `BpmMediateur`
...
│   ├── Abstractions/         # Interfaces client (ICommandHandler, IQueryHandler, IBpmMediateur, IGestionTache)
