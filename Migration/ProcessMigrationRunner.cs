...
public static class ProcessMigrationRunner
{
        ...
        ProcessMigration migration)
    {
        ...
        var targetNodeId = migration.ResolveNodeId(sourceNode.Name);

        // Find target node by name
        var targetNode = targetDefinition.GetNode(targetNodeId);
        if (targetNode == null)
        {
            return MigrationResult.Failed(previousVersion, newVersion,
                $"Target node '{targetNodeId}' not found in target definition.");
        }
